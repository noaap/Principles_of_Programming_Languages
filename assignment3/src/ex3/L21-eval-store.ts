...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore,applyEnvStore, ExtEnv, theGlobalEnv, globalEnvAddBinding, theStore } from "./L21-env-store";
...
const applicativeEval = (exp: CExp, env: Env): Result<Value> =>
    ...
    isVarRef(exp) ? applyEnvStore(env, exp.var,theStore) : 
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses: number[] =map((v:Value)=>{
        extendStore(theStore,v);
        return theStore.vals.length-1 },args);
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses = map((v:Value)=>{
        extendStore(theStore,v);
        return theStore.vals.length-1 },vals);
