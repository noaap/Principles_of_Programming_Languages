...
bind(mapResult(parseL31CExp,map(second,methods)),(lambdas: CExp[])=>makeOk(makeClassExp(map(makeVarDecl,fields),zipWith(makeBinding,map(m=>m[0],methods),lambdas)))):
makeFailure("Failed to parse class");
...
export const unparseClassExp = (cla: ClassExp) : string => 
    `(class (${map((p: VarDecl) => p.var, cla.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`, cla.methods).join(" ")}))`
